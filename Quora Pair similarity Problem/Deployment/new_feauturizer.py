...
def basic_feature_extraction(row):
    ...
    q1_len = len(row['Marking Scheme'])
    q2_len = len(row['Student Response'])
    q1_n_word = len(row['Marking Scheme'].split(" "))
    q2_n_word = len(row['Student Response'].split(" "))

    w1 = set(map(lambda word: word.lower().strip(), row['Marking Scheme'].split(" ")))
    w2 = set(map(lambda word: word.lower().strip(), row['Student Response'].split(" ")))

    q1 = row['Marking Scheme'].lower().replace('?','').replace('.','').replace('!','')
    q2 = row['Student Response'].lower().replace('?','').replace('.','').replace('!','')
...
def extract_features(df_train):
  ...
  basic_features = df_train.apply(basic_feature_extraction, axis = 1)
  basic_features_names = ['q1len', 'q2len', 'q1_n_word', 'q2_n_word', 'word_Common', 'word_total', 'word_share']
  for i, name in enumerate(basic_features_names):
       df_train[name] = list(map(lambda x: x[i], basic_features))
